...
- **[常考面试点]**：PPO 四个模型；SFT 和 RL 的区别；手撕函数
...
- **默认已熟**：SFT 基础（数据格式 / loss masking）和推理基础（generate / sampling，见 [05_01_推理基础.md](../05_推理优化/05_01_推理基础.md)）
- **算法只讲设计层**：每个部件回答"为什么需要它"，公式给形态、不做推导
- **落点是 §5 资源画像**：一个 RLHF（人类反馈 RL，§4 定义）step 里有几个模型、显存占多少、两种负载（训练推理）
...
<img src="../assets/11-rl-basics/fig-02.png" alt="fig-02" style="zoom:40%;" />
