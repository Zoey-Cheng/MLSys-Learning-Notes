...
> **TL; DR**：为什么 SFT 后还要 RL（无负反馈 → 从评价性反馈试错学习）→ RL loop：rollout → reward → advantage → update（= 推理 + 训练交替）→ PPO 各部件为什么存在（advantage / critic / GAE / clip / KL 惩罚）→ InstructGPT 四模型 + 手写 BasicPPO → 资源画像：显存账 + 两种负载的计算特性
...
- **[常考面试点]**：PPO 四个模型各是什么、谁训谁冻结；SFT 和 RL 的区别；advantage 是什么、clip 为什么能限步长；RM 怎么训

本篇是 RL 系列第一篇，系列以 infra 视角为主。Plan的后续篇：02 讲算法演化（DPO / GRPO），03–06 讲训推协同，07 讲 Agentic RL。
...
- **默认已熟**：SFT 基础（数据格式 / loss masking）和推理基础（generate / KV cache / sampling，见 [05_01_推理基础.md](../05_推理优化/05_01_推理基础.md)）
- **算法只讲设计层**：每个部件回答"为什么需要它"，公式给形态、不做推导
- **落点是 §5 资源画像**：一个 RLHF（人类反馈 RL，§4 定义）step 里有几个模型、显存占多少、两种负载各是什么特性。03–06 的系统问题都从这张画像出发
...
- AlphaGo 是最直观的例子：没有人告诉它每步该下哪，唯一反馈是整局的输赢。
...
新的代价也来自这里：反馈延迟到一局结束才来，功劳怎么分回中间每一步成了新问题——这是 advantage（§3.3）存在的根本原因。
...
> 🖼️ **[图待补 fig-02]** 主题：一条 response 在 token 轴上的 RL 映射
> 画面：横轴是 token 序列，左端灰框 prompt，右侧若干方块是逐个生成的 token。第 $t$ 个方块下标 $a_t$（动作=该 token），从 prompt 到该方块左侧的整段花括号标 $s_t$（状态=prompt+前 $t-1$ 个 token）；最末方块上方单独标一个 $r$（整条 reward 只在这出现）
> 意图：让"状态=前缀、动作=下一 token、reward 末位才来"从抽象映射变成一眼可见的序列图

<img src="../assets/11-rl-basics/fig-02.png" alt="fig-02" style="zoom:30%;" />
...
flowchart LR
    ...
    U("update<br/><br/>PPO loss 更新<br/>actor / critic<br/><b>= 训练负载</b>")
...
- **rollout（推理）**：actor 逐 token 自回归生成 response——就是 [05_01_推理基础.md](../05_推理优化/05_01_推理基础.md) 的 generate
...
只有首尾两段是重负载：rollout 走推理、update 走训练。中间 reward / advantage 只对整条 response 各做一次前向，没有自回归循环，很轻。标题的"交替"指的就是首尾这两段。

**一个 RL step 内嵌了一次完整的推理**：rollout 走推理、update 走训练，两种负载交替出现，且共享 actor 的同一份权重。这个耦合是全系列的中心对象。
...
> **逐 token CE 是一般 CE 的退化形态**。一般 CE 对词表求和 $-\sum_v q(v) \log p(v)$。$q$ 为 one-hot 时塌缩成 $-\log \pi_\theta(\text{GT}_t \mid s_t)$，即 pretrain / SFT，$w_t \equiv 1$。$q$ 为 soft 标签（如蒸馏）不塌缩，得全词表求和，写成 KL。
>
> RLHF 没走 soft：仍 one-hot 取一格，只是乘上 $w_t \ne 1$。所以三者共享骨架 $-\sum_t w_t \log \pi_t$，差别只在 $w_t$。
...
> 🖼️ **[图待补 fig-03]** 主题：SFT vs policy gradient——同一个前向反向，监督信号不同
> 画面：上下两行，各画一条 token 序列，每个 token 位置上方一小组柱状图（模型输出的词表概率分布）。上行 SFT：每个位置的分布旁并列一个单柱分布（GT one-hot），两者之间标 CE；下行 policy gradient：序列标"rollout 采出的 y"，各位置只有模型分布、没有 one-hot，整条末尾一个圆圈标量 R（RM 打分），箭头指向整条序列，标"乘在整条 logprob 前的系数"
> 意图：SFT 的监督是每个位置一个 one-hot 分布、逐位置算 CE；policy gradient 没有 one-hot，监督只是整条一个标量；两者的前向反向机制相同
...
问题：$w_t$ 恒 $= R$，写对和写错的 token 同奖同罚，不合理。支线一（§3.2–3.4）把 $w_t$ 换成逐位置的量；另一个因子 logprob 项的问题是复用——rollout 贵，一批数据想多次更新，支线二（§3.5）换成 ratio 并 clip。分步见 §3.0 末尾的图。
...
<img src="../assets/11-rl-basics/fig-05.png" alt="fig-05" style="zoom:50%;" />
...
<img src="../assets/11-rl-basics/fig-09.png" alt="fig-09" style="zoom:45%;" />
...
> 🖼️ **[图待补 fig-11]** 主题：RM 训练——一个 pair 两次 forward，比较只在 loss
> 画面：顶部一个 prompt 方块，向下分两支。上支拼成完整序列 prompt + $y_w$（chosen，绿框），下支 prompt + $y_l$（rejected，红框），两条都画成一串 token。两支各输入一个 RM 方块，两个 RM 方块用虚线连起并标"同一模型 · 权重共享（backbone + 标量 head）"。每个 RM 只从序列**末位** token 引出一个标量（前面位置画灰、不引出）：上支得 $r_w$、下支得 $r_l$。两个标量汇入一个 loss 框，框内写 $-\log\sigma(r_w - r_l)$，旁注"$\sigma(r_w-r_l)=P(y_w\ 更好)$，label 恒为 w 赢，梯度推 $r_w>r_l$"。底部一行注：每次 forward 都是"一条序列 → 一个标量"，两条是同一模型跑两遍，"比较"只在 loss 里拼出来
> 意图：让"模型 forward 始终 pointwise、pairwise 只活在 loss""标量只取末位"两点一眼可见

<img src="../assets/11-rl-basics/fig-11.png" alt="fig-11" style="zoom:50%;" />
...
把 §4.2 的表和 §3.6 的计算链落成能跑的代码，分两档：**高频手撕的纯函数**（面试重点，练到可默写），**模型组件 + loop 骨架**（熟悉结构、能调用即可）。toy 规模（词表 100、hidden 64），CPU 直接跑通；完整可运行版见 [11_basic_ppo.ipynb](https://github.com/Zoey-Cheng/MLSys-Learning-Notes/blob/main/code/11_basic_ppo.ipynb)。真实系统的工程项省略。
...
**后两个是参考 doc 单列的 "PPO" 一节手撕的核心**，前两个是 RLHF 特有的前置（纯 PPO 没有）。下面按这个分界拆成两块。
...
- **变长 prompt 的 attention mask**：BasicPPO 已用 EOS + response_mask 处理变长 response，但 prompt 都等长，省了另一半——真实 batch 里 prompt 也长短不一，要左 pad 并加 attention mask 让注意力跳过 pad token。toy 的 `MiniLM` 只有 causal mask（管自回归），没有这个 padding mask。
...
<img src="../assets/11-rl-basics/fig-13.png" alt="fig-13" width="70%" />
