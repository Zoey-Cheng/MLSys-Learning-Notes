...
- **[Quick Ref for 手写code]**：mini-lora-sft ｜ [ipynb](https://github.com/Zoey-Cheng/MLSys-Learning-Notes/blob/main/code/05_mini_lora_sft.ipynb) ｜ [Colab](https://drive.google.com/file/d/1NrDWiGrWPoRrk2yszFXIkDe-DeyKW7B0/view?usp=drive_link)
...
- 不展开模型结构和训练通用流程（forward / loss / lr / 评估）：回顾见 [Transformer](https://zoey-cheng.github.io/MLSys-Learning-Notes/01_模型基础/01_01_Transformer.html) 和 [预训练 Pretrain](https://zoey-cheng.github.io/MLSys-Learning-Notes/03_训练方法/03_01_Pretrain.html) 篇
...
直觉：模型"读问题"时不需要被惩罚：prompt 是用户给的，不是模型生成的。强迫模型预测 prompt 文本浪费容量，还会损害模型组织答案的自由度。SFT **只让模型为自己产出的部分（response）负责**。

**[为什么学习率差一个量级]**

Pretrain 是从随机权重出发，要大 lr 把权重快速推到合理位置。SFT 是在已经训好的权重上小幅调整，lr 太大会**灾难性遗忘**：模型丢掉预训练学到的世界知识，只剩 SFT 数据分布内的行为。所以 SFT 用 1e-5 ~ 5e-5（仍然是 warmup + cosine decay）。



## 2. Full SFT 处理流程

大厂用开源模型训自家业务模型时，SFT 阶段基本都是 **全量微调 (Full SFT)**，即更新模型的全部权重：算力充足时效果上限最高，资源受限才退到 PEFT（§3）。
...
ChatML 是通用对话的标准模板，业务上常在它基础上扩展——§2.2 是 token 粒度的扩展，这里是**整段模板结构**的扩展。常见需求：
...
tokenizer 加了新 token 后，模型端要相应处理。最直接受影响的是 **embedding 矩阵**和 **lm_head**——它们都是按"词表大小 × hidden dim" 形状存的。
...
- `resize_token_embeddings` 按 model 实际形状走（hf 默认），自动对齐改造后的维度，不要硬编码标准 d
- 新 token 行的均值初始化基于**当前**的 V 行，不要从原始开源模型拷贝向量再截断
...
显存收益比 PEFT 弱——梯度和 optimizer state 还是按"未冻结部分的参数量"算，且模型 weight 要全量加载。但实现成本低、不引入新模块，是 Full SFT 显存紧张时的常用手段。
...
7B 单卡装不下，70B 要 ~1 TB 显存 + ZeRO-3。SFT 数据量小，计算时间不是主要限制，**显存才是**。这就是 PEFT 的动机：不是为了快，是为了在显存小的卡上能跑。
...
新增参数 0.01% ~ 3%、实现极简。但**小模型效果差**（10B 以下基本不如全量），且推理时序列变长更慢。工业上基本被 LoRA 取代，只在 100B+ 大模型、显存极紧的场景偶尔出现。
...
**主要缺点**：每个 block 多两次小矩阵乘 + 中间非线性，**无法吸收进原权重**，推理永久多一份开销。这是它被 LoRA 取代的根本原因。

## 4. LoRA：资源紧时的首选

> 《LoRA: Low-Rank Adaptation of Large Language Models》Microsoft 2021 [[link]](https://arxiv.org/abs/2106.09685)

**LoRA 不是工业级 SFT 的首选**——大厂训自家 instruct 模型用的还是 Full SFT。LoRA 的适用场景是**资源受限的 SFT**。
...
**LoRA 故意不加中间非线性**：BA 在数学上就是一个矩阵，可以**合并回原 W**。这是它相对 Adapter / Prefix 的关键优势。
...
LoRA 的主要收益不在 forward 算量，而在**反向 + 优化器状态的显存**：原 $W_0$ 完全冻结，只有 A 和 B 需要梯度和 Adam 状态。这就是单卡 24 GB 也能跑 7B LoRA 的原因。
