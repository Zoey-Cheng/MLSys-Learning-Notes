...
- **朴素层（够用）**：动态 dispatch——按 expert 把 token 聚一起，每个 expert 实际收几个算几个。代码就是个 per-expert for-loop（N 次独立 dense GEMM），不 drop 不 padding。Mixtral / 早期实现就是这个，§2.3 / §3 的代码都是它。注意这一层 **PyTorch 看到的只是 N 次 GEMM 发射，不是 block-sparse**
- **kernel 优化层（更快）**：直到 2022 年 Megablocks 才把那 N 个变长 GEMM 真正融成**一次 block-sparse GEMM**——自定义 kernel 吃变长 block，省掉 N-1 次 kernel launch。这才是"block-sparse GEMM"本体，PyTorch 算子不自带
