...
- **[Quick Ref for 手写 code]**：mini MoE Qwen3 ｜ [ipynb](https://github.com/Zoey-Cheng/MLSys-Learning-Notes/blob/main/code/06_mini_moe_qwen3.ipynb) ｜ [Colab](https://drive.google.com/file/d/19DwqNBulYpXuyedOMbglgauXsB183Oil/view?usp=sharing)
    ...
    - 可运行代码取 Qwen3-MoE， MoE部分（手写）+ Qwen3 MHA（调用） 拼成完整一层，与 HF 版对齐验证正确性
    
- **[常考面试点]**：结构 / batch process / 并行部分都常考
...
读完 [01-Transformer.md](01_01_Transformer.md) 知道 LLaMA Block：每层的主要结构是 Self-Attention → FFN (MLP)。其中FFN 是参数大头，占~70%。
...
- **1. 什么是 MoE**：结构 / 公式 / 1991 起源 / batch process 这个早期工程难点
...
LLaMA 的 transformer block 里，~70% 的参数在 FFN / MLP。从 GPT-2 到 LLaMA-3 这个比例一直成立。

自然的问题：每条样本大概率只用得到其中一部分参数。能不能做到"参数容量很大、单次前向只用一小部分"？这就是稀疏激活的想法，MoE 由此而来。

这个想法 1991 年《Adaptive Mixture of Local Experts》就提出了。直觉也很自然：不同 expert 各自负责一个子领域，router 为每个 token 选出最相关的几个，不必让一份参数同时承担所有知识。
...
- **Router / Gate**：一个 `Linear(d, N)`，参数仅 d × N（d = hidden_dim），给每个 token 打 N 个分
...
对每个 token（d = hidden_dim），forward 分三步：
...
1991 年就有的想法，为什么这两年才火起来？还记得 24 年春天，我们 MLSys 课的大作业有一个选项：给 MoE 加 batch process。小组研究了一晚上，最后换了题目 lol（当时业界还没有出名的通用解法）。

卡点不在算法，在工程：**GPU 适合大规模矩阵乘，原生 MoE 的路由结果组不成单次大 GEMM**。

dense FFN 下，一个 batch 的 token（eg: batch_size = 8）一起输入同一个 FFN，是一次规整 GEMM。MoE 把这件事拆散了——同 batch 的 token 可能去往不同 expert：
...
**Shazeer 2017 给出"半解"——Dispatch / Combine + Capacity**：把 token 按 expert 重排，每个 expert 接收的 token 数强行统一成长度 C。这样每个 expert 都是一次规整 GEMM。但 C 切到多少都不完美：
...
PyTorch 生态已经不用它了。但 JAX/XLA 是静态图、不支持动态 shape，所以今天 TPU + JAX/XLA 的大规模训练中这个解法仍在使用。

**[解法B：动态dispatch（PyTorch通解），上面再叠 block-sparse GEMM 优化]**

- 想法和解 A 一样，也是 dispatch 到 per-expert。但 PyTorch 是动态图，不需要预先定义统一长度 C。

- 加速并不发生在 PyTorch 层——代码上仍是 N 个变长 GEMM。真正提速靠 kernel 层：把多个变长 GEMM **融合成单个 kernel 的 Block Sparse GEMM**（支持动态 shape）。

具体的稀疏矩阵乘算法留到算子篇展开。当代 MoE + NVIDIA GPU 用的都是这套逻辑。

学术解 ≠ 工程可用：主流框架把解 B 消化成 import 即用，又花了 1~2 年——所以 24 年初的我们还找不到现成做法 orz
...
| 2025    | **Qwen3-MoE**       | 反向简化——最简约的教科书式解法                       | 多档            |
...
MoE 是修改 FFN 得到的，先回忆 LLaMA Decoder Block 里的 FFN 结构。

回顾 [01-Transformer.md §4.2](01_01_Transformer.md)：FFN 采用"升维-降维"结构（hidden_dim 通常设为 `4 * embedding_dim`）；LLaMA 版用 **SwiGLU** 替代了 ReLU，SwiGLU 可以调用 **SiLU** 函数实现：
...
- vs **Mixtral（§3）**：MoE block 层面**没有算法区别**——Mixtral 就是 BasicMoE 的真实规模版
...
- **dispatch 写法**：per-token 双层 for 是原始版，per-expert 是实际用的版。但它真正的性能收益其实落在**算子层面的加速**→ **§2.3**
...
per-expert 写法在 PyTorch 层本身没有收益——N 个变长 GEMM 仍是 N 次独立 kernel launch。真正提速靠**融合 kernel**：block-sparse / grouped GEMM（Megablocks），把 N 次合成一次 launch。

### 2.4 进阶：Aux Loss - Expert Balance

前向 code 看不出 loss 计算的特点，所以单独开一小节。

均衡有两种粒度，device 粒度到 §4.5 再说。不想看数学的话，过一眼定义即可跳过本节：
...
回忆梯度下降 $\theta \leftarrow \theta - \mathrm{lr} \cdot \partial L / \partial \theta$：正梯度意味着优化器会把这个量往下推。所以 $f_i$ 越高，$P_i$ 被压得越低；再经 softmax 反传，对应 expert 的 logits 被拉低——这就是"惩罚"。
...
Dead expert 的 $f_k = 0$ 是所有 expert 里最低的，一定低于加权平均，于是 logits 被推高、下次更可能被选中。softmax 的耦合让"不被惩罚"直接变成了"奖励"。
...
> **Loss侧的历史归属**：负载均衡 + aux loss 概念是 **Shazeer 2017**提出；**DeepSeek-MoE V1** 沿用，加了 device-level（§4.5）；**V2/V3** 又加了额外的loss设计用于约束通信量。
...
和 §2.2 BasicMoE 算法上几乎一样。BasicMoE 并不是照着 Mixtral 写的——只是把经典设计加到一个基础 MoE 上，结果就和 Mixtral 一致了🤔。

代码里只有 dispatch 的**写法**不同（换成更标准的 `expert_mask` + `index_add_`），过一眼即可。和 §2.2 不一样的行用 `# ≠§2.2` 标出：
...
DeepSeek 2024.01 发表 *DeepSeekMoE*，配套发布 16B / 145B 两个规模的模型。它定义的设计被之后多家沿用——Qwen2-MoE、Kimi-K2、DeepSeek-V2/V3 都走这条路线（Qwen3-MoE 是反例，详见 §5）。
...
每条按 **〔针对什么〕→ 改了什么 → 目的** 来读（仅供参考，可跳过）：

**DeepSeek-V2（2024.05）——主攻"推理省显存 + 限制通信量"**

- **MLA**〔针对 KV cache〕：把 KV cache 联合压成一个低秩隐向量、用时再还原，KV cache 显存大降。本质是 attention 优化
- **Device-Limited Routing**〔针对通信·发送端〕**——硬路由约束，不是 loss**：forward 里直接限制每个 token 只能从 Top-M 个 device（M=3）里选 expert，控制 All-to-All 通信量
- **Comm Balance Loss**〔针对通信·接收端〕**——loss新增项**：Device-Limited 只硬性管住了发送上界、没管接收；这项加进总 loss，靠梯度**鼓励** router 让每张卡**收到**的 token 数也均衡，避免单卡接收流量过高
...
- **Hybrid Attention**〔针对 long-context attention〕：CSA + HCA + DSA 三种 attention 组合替换 MLA，1M context 下 V4-Pro 只要 V3.2 的 27% FLOPs / 10% KV cache
...
**[和 DeepSeek 反着来的]**：移除 shared expert，均衡只停在 Switch aux loss（没有 DeepSeek 那一系列限制通信量的 loss）
...
Qwen 相比 DeepSeek 系列，MoE 几乎没加额外设计，反而是简化。那效果如何、选型时为什么有时不选 DeepSeek 而选它？（我们组开源部分用的就是 Qwen 系列）
...
- **DeepSeek 系列（§4）**：V1 三个核心改动——细粒度 Expert（提容量）+ 共享专家（降冗余）+ device-level balance loss（device 负载均衡）；V2/V3/V4 一路加（MoE 本体没大改，工程件越来越多）
- **Qwen 系列（§5）**：反向演化——只留了细粒度expert 的简约设计。胜在档位齐全（0.6B–235B、dense+MoE）+ 生态易用

贯穿全篇的两个核心问题：

- **batch process（怎么让稀疏路由跑在 GPU 上）**：Shazeer capacity 是带 drop/padding 的半解，JAX/XLA 因静态图特性仍在用 capacity_factor；PyTorch 的默认解收敛到动态 dispatch，真正提速在算子层的 block-sparse / grouped GEMM（MoE 由此兴起）
