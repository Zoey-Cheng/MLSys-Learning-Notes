...
- **[Quick Ref for 手写 code]**：mini MoE Qwen3 ｜ [ipynb](https://github.com/Zoey-Cheng/MLSys-Learning-Notes/blob/main/code/06_mini_moe_qwen3.ipynb) ｜ [Colab](https://drive.google.com/file/d/19DwqNBulYpXuyedOMbglgauXsB183Oil/view?usp=sharing)
    ...
    - 可运行代码取 Qwen3-MoE， MoE部分（手写）+ Qwen3 MHA（调用） 拼成完整一层，与 HF 版对齐验证正确性；选它是因 block 代码最精简——相对 §2.2 BasicMoE 只多 `norm_topk_prob` 开关 + 细粒度专家（都是小改动）
...
但其实加速的并不是Pytorch层的算法，看code仍然是N个变长 GEMM。真正提速靠的是kernel层的发明：将多个变长GEMM**融合成单个 kernel的 Block Sparse GEMM**（吃动态shape！）
...
**Mixtral** 是第一个知名开源MoE，在它之前，batch process的两个解、AUX Loss的出现都是里程碑。

**DeepSeek系列**则是开创了当代MoE走上应用的先河，V1 - V4逐渐复杂；**Qwen** 则是开始和Deepseek很像，但是往后却走了简化路线。

接下来从：Basic Moe -> Mixtral -> DeepSeek系列 -> Qwen逐一展开。
...
- **Qwen2-MoE**：相比上一代规模放大；shared expert 上加了一个 sigmoid 门控 `shared_expert_gate`（`σ(Wx)·shared(x)`），DeepSeek 是无条件恒加，这里让 shared 的贡献也可调
...
- **负载均衡（怎么不让 expert 死掉）**：基础解是 Switch expert-level aux loss（§2.4）；DeepSeek 在上面叠 device / comm balance（V1/V2），V3 换成 aux-loss-free 动态 bias；Qwen 停在基础 aux loss 不再加

EP 和相关 block-sparse GEMM 算子，后面再说吧（

## 7. 代码实现

可跑代码取 Qwen3-MoE 的 block，再配上 Qwen3 的 MHA，拼成完整可跑的一层。和直接调HF版结果对齐，验证了正确性：

选 Qwen3-MoE 是因为它的 block 代码最精简，相对 §2.2 BasicMoE 只多：(1) `norm_topk_prob` 开关，(2)细粒度专家。
