...
- **§2 手写最小 VLM**：LLaVA 式 forward 骨架——Vision 和 LLM冻结、直接call，训 MLP Connector
...
- **Vision Encoder（答 Q1 + Q2a）**：把图编码成一串 visual 特征，**训练时一般全程冻结**
  ...
  - 但**预训练目标**必须用 CLIP/SigLIP对齐图文（§1.3，答 Q2a），feature 才"懂语言" + 能和后面的 LLM 接得上
...
三段里 (1) LLM 拿来即用；(2) Vision Encoder 是从视觉领域借的（且训练时冻着）；(3) 真正从头训的只有 Connector——所以 LLaVA 范式只用 ~1M 量级数据就能训好。VLM 设计空间也就集中在 Connector 和"怎么喂图"上（§3 工程问题）。
...
模型中，大部分时候是直接用对齐好的ckpt的，但有时候也会在次基础上解冻训练（见Qwen系列）。
...
结构和 CLIP 完全一样，只换"怎么对比 M 和 I"。改的动机是 CLIP loss 在工程上有两个痛点（CLIP 强依赖大 batch——论文 batch = 32k，负样本越多对比信号越强）：

- **softmax 是 batch 级归一化**：softmax 要看遍整个 batch；分布式必须先 **all-gather**
- **N×N 相似度矩阵 → 显存 `O(N²)`**：batch 一大就爆

**SigLIP 改法：softmax → sigmoid**——把"N 路分类"变成 **N² 个独立二分类**。M 的每格 `M[i,j]` 单独问"配对吗？" -> 每格独立
...
- **Vision Tower (全程冻结)**：直接call CLIP ViT-L/14-336，输入 `[B, 3, 336, 336]`，按 §1.2 的 patchify  → ` 576 个 patch` → 输出 `[B, 576, 1024]`（576 个 patch token，d_vision = 1024）。
...
| 路线 | 怎么做 | 谁用 | 代价 |
|---|---|---|---|
| **原生分辨率(NaViT)** | 保留原图分辨率和长宽比直接切 patch → 不同图得到不同 patch 数（变长序列） | Qwen2-VL 起 | ViT 原版绝对位置编码废了，必须换（→ §3.2） |
...
§3.1 动态分辨率的副作用：一张高分辨率图能切出几千个 patch，全塞进 LLM 序列又长又贵（attention 是 `O(T²)`）。投影进 LLM 之前要先压一道。
...
Qwen 这条线从 2023 走到 2026 跨了五代，几乎踩过 §3 每一个工程问题。
...
**DeepEncoder 内部结构**（Base 1024² 配置为例），简单看看吧：
