...
- **[面试点]**：看懂 MiniVLM 部分应该就够了——Vision 和 LLM 部分相对冻结，设计核心是 Connector



## 前言

读完 [01_01_Transformer.md](01_01_Transformer.md) 知道：LLM 只认 embedding 向量序列，不在乎这个向量从哪来。VLM 就是抓住这一点——LLM 本体不动，前面加 pipeline 把"图"转化成能输入 LLM 的 token。本篇大部分内容讲这段 pipeline 怎么搭。
...
- **§2 手写最小 VLM**：LLaVA 式 forward 骨架——Vision 和 LLM 冻结、直接调用，训 MLP Connector
- **§3 走向 real-world**：固定分辨率不够用——动态分辨率 / 2D-RoPE & M-RoPE / token 压缩
- **§4 Qwen-VL 系列**：经典版 Qwen2-VL（= §2.1 + 补§3）
- **§5 DeepSeek 系列**：经典版 DeepSeek-OCR（视觉 token 当文本压缩器）

Qwen 线延续简约设计，DeepSeek 线自定义组件更多——和 MoE 篇的观感一致。
...
- **Vision Encoder（答 Q1 + Q2a）**：把图编码成一串 visual 特征，**训练时一般全程冻结**
  ...
  - **预训练目标**用 CLIP/SigLIP 做图文对齐（§1.3，答 Q2a）。对齐后的 feature 带语言语义，才能接进后面的 LLM

- **Connector / Projector（答 Q2b）**：
  - ViT 出来的 feature 维度（如 1024）和 LLM 的 hidden_dim（如 4096）不一样，要 MLP 投过去
  - **这是整套结构里唯一从头训的部件**——也是 VLM 设计里改动最频繁的（§1.4）。

- **LLM**：直接拿现成的文本基座（Vicuna / LLaMA / Qwen / DeepSeek），结构上几乎不动。训练初期冻结、后期 SFT 时才解冻调

三段里，LLM 拿来即用；Vision Encoder 借自视觉领域，训练时冻结；真正从头训的只有 Connector。所以 LLaVA 范式只用 ~1M 量级数据就能训好。VLM 的设计空间也集中在 Connector 和图像输入方式上（§3 工程问题）。

### 1.2 ViT: 图变 token (通常冻结)

**ViT (Vision Transformer)** = 把图先切成 token、再走一遍纯 Transformer 主干。拆成三步（以 ViT-B/16 处理 `224×224×3` 为例）：

- **入口**：Conv Patchify + `[CLS]` + 位置编码
  - **Conv Patchify**：图过一次 **无重叠的 Conv2d，每个输出位置就是一个 token——业界把它叫 patch**
  - **`[CLS]` token**：BERT 沿用过来的"分类位"——一个加在序列最前面的可学习向量，N 层 attention 之后它的 hidden 就是整张图的"摘要"（出口拿它过分类头）。
  - **位置编码**：一个可学习的位置矩阵，**逐元素**加到序列上。和 LLaMA 的 RoPE（每层旋转 Q/K）不同，ViT 这里是进 Transformer 之前**一次性加好**，后面 N 层都不再管位置。
- **主干**：N 层 Transformer
- **出口**：**取 `[CLS]` 过分类头**（`Linear(d, K)` → K 类 logits）——结构和 LLM 末尾的 **LM head** 完全一样（K = `vocab_size`）。

<img src="../assets/09-vlm/fig-02.png" alt="image-20260624135126530" style="zoom:40%;" />

图里数字按 ViT-B/16 走（hidden_dim **768**、N = 12 层、分类头到 ImageNet 1000 类）。

**一个事实约束**：ViT 没有 CNN 的局部性归纳偏置，所以只有在超大规模数据上预训练才会显著超过 CNN（原文用 JFT-300M，3 亿张图）。

### 1.3 CLIP / SigLIP: 图文语义对齐 (通常冻结)
...
实际模型大多直接用对齐好的 ckpt，有时也在此基础上解冻继续训（见 Qwen 系列）。

**[CLIP（OpenAI, 2021）：图文对比对齐]**

核心 idea：放弃人工分类标签，用 4 亿对 `(图, 描述文本)` 配对来训——让"猫的图"和"a photo of a cat" 文本特征靠近。

> 如果不想了解训练设计，看到这里已经可以跳过了。

CLIP 结构 = 双塔 + 共享投影空间——两个 encoder 独立处理图和文，但都投到同一个 d 维空间，可以直接算余弦相似度：

- **Image Tower**：`[B, 3, H, W]` 图 → ViT → `[B, hidden_vit]` → **Linear 投影到 `[B, d]`** → **L2 归一化**。
- **Text Tower**：`[B, T]` 文本 token → Transformer 主干 → 取 `[EOS]` 位置的 hidden `[B, hidden_txt]` → **Linear 投影到 `[B, d]`**（**同一个 d**）→ **L2 归一化**。

> **`[EOS]` 不是终止符吗？** 在 CLIP 的 text encoder 里它还兼"全句摘要位"——因为 text encoder 是 **causal**（看前不看后），只有最末尾的 `[EOS]` 看遍了全句。
>
...
> - **为什么 CLIP 用 L2**：余弦相似度公式 `cos(v,t) = (v·t)/(‖v‖₂‖t‖₂)`，用L2 **余弦退化成纯点积** `v·t`，方便计算

<img src="../assets/09-vlm/fig-03.png" alt="image-20260627211212977" style="zoom:50%;" />

- **训练 loss（对称 InfoNCE，只训练时用）**：
  - **GT = N×N 单位矩阵 `I`**（对角线 1，其它 0），训练目标 = 把 M 推成 I 的形状（要的只是相对排序——`M[i,i]` 在同行同列里最大，不要求绝对值 = 1）。
...
- **训完后 / VLM 用法**：只用图像塔（文本塔丢掉），它输出的 feature 已"指向"语言空间



**[SigLIP（Google, 2023）：同思路换 loss（softmax → sigmoid），工程化]**

结构和 CLIP 完全一样，只换"怎么对比 M 和 I"。改的动机：CLIP loss 强依赖大 batch（论文 batch = 32k，负样本越多对比信号越强），工程上有两个痛点：

- **softmax 是 batch 级归一化**：softmax 要看遍整个 batch；分布式必须先 **all-gather**
- **N×N 相似度矩阵 → 显存 `O(N²)`**：batch 一大显存就不够

**SigLIP 改法：softmax → sigmoid**——把"N 路分类"变成 **N² 个独立二分类**。M 的每格 `M[i,j]` 独立判断"是否配对"。
...
§1.3 训出了可以直接用的 vision encoder（CLIP/SigLIP 图像塔），能输出"带语义"的视觉 feature。但这些 feature 在自己的 `d_vision` 空间里（如 **1024**），和 LLM 的 `d_llm` 词嵌入空间（如 **4096**）——维度对不上、分布也不一样，没法直接拼进 LLM 输入。还差一个**变换**把它送进 LLM 空间，让 LLM 当普通 token 读。这个变换就是 **Connector / Projector**。
...
- 最朴素：一个可训练 MLP 把每个 patch feature 独立投到 LLM 词嵌入空间。
- 整套结构里只有这个 MLP 是从头训的——ViT 和 LLM 都用预训练好的现成模型。这是 LLaVA 范式数据高效的根本原因。

**为什么胜出**：

- **信息无损**：patch token 数原封不动搬过去，细粒度任务（OCR、文档）不受影响。
- **训练稳定 + 数据高效**：MLP 是最简单的可训练变换，从头训的参数量极小。LLaVA 约 **1M** 量级图文对就训得很好，而 Q-Former 路线动辄 **1B+**（因为 Q-Former 自身那套 query + cross-attn 参数也得从头训）。
- **要压 token 数也不必上 Q-Former**：简单的 2×2 合并 / average pooling 就能顶上（§3.3 PatchMerger），避免 Q-Former 复杂结构。

到这里 **三段式全齐**：vision encoder (VIT + CLIP/SigLIP) + connector (MLP) + LLM (Vicuna/LLaMA/Qwen/...) = **LLaVA 架构**。下一节 §2 把它写成代码，就是个最小 VLM。
...
**先说三段都用什么具体模块**（以 LLaVA-1.5 为例），拼起来就是 forward：

- **Vision Tower (全程冻结)**：直接调用 CLIP ViT-L/14-336，输入 `[B, 3, 336, 336]`，按 §1.2 的 patchify → `576 个 patch` → 输出 `[B, 576, 1024]`（576 个 patch token，d_vision = 1024）。
...
- 在输入文本里放 **N_patch 个连续 `<image>` 占位 token**，先正常 embedding 查表得到序列
- 再把这 N 个占位位置的 embedding **替换**成投影后的 N 个视觉 token。

这样后面 LLM 完全按普通 causal LM 跑，根本不知道某些 token 是从图来的。
...
class MiniVLM(nn.Module):
    ...
    def forward(self, input_ids, pixel_values, image_token_id):
        ...
        return self.llm(inputs_embeds=inputs_embeds)         # ← 调用 forward
```
...
真实场景里有 4K 文档、竖屏视频、超宽表格。固定分辨率要么把小字压糊，要么强行改长宽比造成形变。真实 VLM 普遍要解决**三个相互关联的问题**：
...
- **低分辨率丢细节**（小字、图表线条压糊）
- 强行改长宽比导致**形变**（长文档被拉成正方形）
- 而且**不同图信息密度差异大**（同一个固定 token 数既浪费又不够）

两条解法：

| 路线 | 怎么做 | 谁用 | 代价 |
|---|---|---|---|
| **原生分辨率(NaViT)** | 保留原图分辨率和长宽比直接切 patch → 不同图得到不同 patch 数（变长序列） | Qwen2-VL 起 | ViT 原版绝对位置编码失效，必须换（→ §3.2） |
| **动态切图(Dynamic Tiling)** | 切成若干固定尺寸 tile + 1 张全局缩略图，各自编码再拼 | LLaVA-NeXT、InternVL、DeepSeek-VL2 | tile 边界处可能丢上下文；好处是复用现成的固定分辨率 ViT |

两条路殊途同归：`token 数 ∝ 图实际大小`。代价：token 可能很多——引出 §3.3。

### 3.2 位置编码：1D → 2D / M-RoPE

**为什么必须重做**：

- §3.1 动态分辨率后，ViT 原版的可学习绝对位置编码失效（尺寸一变维度对不上）。
...
**[ViT 内部] 2D-RoPE**：head 维度切两半，一半按 patch x 坐标旋转、一半按 y 坐标旋转——有了 2D 相对位置，**任意分辨率都能用**。
...
§3.1 动态分辨率的副作用：一张高分辨率图能切出几千个 patch。全部进入 LLM 序列，序列长、计算贵（attention 是 `O(T²)`），投影进 LLM 之前要先压缩一道。
...
**重要**：PatchMerger 就是 §2 那个 projector——压缩和"投到 LLM 空间"合并成一步。
...
Qwen 这条线从 2023 走到 2026 跨了五代，把 §3 的每个工程问题都处理了一遍。

和 [01_02_MoE.md](01_02_MoE.md) §5 一样**挑一代当经典展开——选 Qwen2-VL**：它是 §3 三件事第一次被一次性补齐的转折点，结构最有代表性，之后各代都是 "Qwen2-VL + X" 的增量。
...
- **LLM 位置编码**：1D-RoPE / **M-RoPE 3D**（§3.2）/ Interleaved-MRoPE（= M-RoPE 频段交错版，§4.2）
...
```python
# 调用: vision_tower 是 Qwen 自己 CLIP-init 重训出来的 675M ViT，内部用 2D-RoPE + NaViT
...
训练策略上还多一处差别：§2.1 冻 ViT 只训 MLP；Qwen2-VL 因为 ViT 内部结构改了（2D-RoPE 换 1D pos embed），必须**解冻让 ViT 一起训**。



## 5. DeepSeek 系列

DeepSeek 这条线和 Qwen 路线不同：Qwen 补通用 VLM 能力；DeepSeek 越往后越聚焦在一个问题上——视觉 token 能压到多小。挑一代当经典展开，选 DeepSeek-OCR：它把"**光学压缩长上下文**"做成了系统方案（详 §5.1 表）。

（个人感觉学习价值不如 Qwen-VL 线。）
...
**和 §2.1 MiniVLM 的关系**：forward 骨架完全一样，唯一替换 `self.vision_tower = DeepEncoder()`——所以本节不展开 code，新东西全在 DeepEncoder 内部。

**DeepEncoder 内部结构**（以 Base 1024² 配置为例）：

<img src="../assets/09-vlm/fig-09.png" alt="image-20260628211129057" style="zoom:40%;" />

### 5.3 其它 VLM 线

补几条 Qwen / DeepSeek 之外的 VLM 线：
...
- 核心是训 Connector，Vision 和 LLM 相对固定 / 冻结
- 具体模型的差异集中在 Vision Encoder（整体换 / 动态分辨率）和位置编码（2D-RoPE / M-RoPE）
