...
默认已熟悉训练流程和 Transformer / attention / MHA（Transformer 篇讲过），只在需要时回顾。作为推理优化系列的第一篇，它把后面每篇共享的底座讲清楚：

- **prefill / decode**：推理相对训练的计算流差别
- **推理性能指标**：TTFT / TPOT / 吞吐 / SLO / Goodput
- **KV Cache**：decode 对前文的"记忆"载体 -> 怎么存、占多少 -> 衍生的优化方向（后续篇）
...
- **Prefill**：整段 prompt（$n$ token）并行前向，产出整段的 KV cache 和第一个生成 token。
- **Decode**：之后每步只 forward 1 个新 token，读已缓存的 KV，生成 1 个新 token，并把它的 KV 追加进 cache。

两者计算特性相反，根源在 attention 计算的形状（batch=1）：
...
**KV cache 因此既占显存、又占带宽**，是推理优化的主要对象。
...
**分位数比平均更重要**：生产里常说的**长尾**指延迟分布的末端——少数最慢的请求。Mean 会被长尾拉偏但定位不到它，分位数才能抓到。
...
先回顾 MHA：三步公式，两次 matmul 夹一个 softmax：
...
KV cache 即每层 attention 的 $K, V$ 常驻 HBM，每层一份。流程如下（括号内步骤与训练版 MHA 相同）：
...
attention 唯一的硬约束来自**残差**：输出要能加回输入 `x + attn(x)`，所以输出维度必须 = $d_{model}$（即 **hidden_dim**，模型设计时确定，如 Llama-7B = 4096；见 [01_01_Transformer.md §2.4](../01_模型基础/01_01_Transformer.md)）。**这个约束只管输出，不管中间怎么算。**
...
完整版：[08_mini_inference.ipynb](https://github.com/Zoey-Cheng/MLSys-Learning-Notes/blob/main/code/08_mini_inference.ipynb) ｜ [Colab](https://drive.google.com/file/d/1Zlicq3BShtFv4iWECkbHSqJ2iNZT42xW/view?usp=sharing)（免费 T4 可跑）
